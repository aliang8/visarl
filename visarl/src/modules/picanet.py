...
class Unet(nn.Module):
    def __init__(
        ...
        self.cfg = cfg
        self.num_enc_out = len(cfg["Size"])
        for i in range(self.num_enc_out - 1):
    ...
    def forward(self, *input):
        ...
        # print(f"encoder took {time.time() - start} seconds")

        dec = None
        pred = []

        for i in range(len(self.cfg["en_indices"])):
            # start = time.time()
            inp_ = en_out[self.cfg["en_indices"][i]]
            dec, _pred = self.decoder[i](inp_, dec)

            # print(f"layer {i} took {time.time() - start} seconds")
            pred.append(_pred)
        loss = 0
        if not test_mode:
            for i in range(self.num_enc_out):
                loss += (
                    F.binary_cross_entropy(pred[self.num_enc_out - 1 - i], tar)
                    * self.cfg["loss_ratio"][self.num_enc_out - 1 - i]
                )
                # print(float(loss))
                if tar.size()[2] > 28:
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.conv6 = nn.Conv2d(
            512, 1024, kernel_size=3, stride=1, padding=12, dilation=12
        )  # fc6 in paper
...
class DecoderCell(nn.Module):
    def __init__(self, size, in_channel, out_channel, mode):
        ...
        if not mode == "C":
            self.conv2 = nn.Conv2d(
                2 * in_channel, out_channel, kernel_size=1, padding=0
            )
    ...
    def forward(self, *input):
        ...
        if dec.size()[2] * 2 == en.size()[2]:
            dec = F.interpolate(
                dec, scale_factor=2, mode="bilinear", align_corners=True
            )
...
class Renet(nn.Module):
    def __init__(self, size, in_channel, out_channel):
        ...
        self.vertical = nn.LSTM(
            input_size=in_channel, hidden_size=256, batch_first=True, bidirectional=True
        )  # each row
        self.horizontal = nn.LSTM(
